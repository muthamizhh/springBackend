...
public class App {
    public static void main(String[] args) throws InterruptedException {
        ...
        devTools.addListener(Network.responseReceived(), event -> {
            ...
            // Only capture the specific POST response
            if (url.contains("https://httpbin.org/post")) {
                try {
                    var body = devTools.send(Network.getResponseBody(requestId));
                    String responseBody = body.getBody();
                    ...
                    System.out.println("Failed to get body for: " + url);
                }
            }
        });
        ...
        // Submit the form
        driver.get("https://httpbin.org/forms/post");
        driver.findElement(By.name("custname")).sendKeys("helo");
        driver.findElement(By.name("custtel")).sendKeys("12345");
        driver.findElement(By.name("custemail")).sendKeys("hello@gmail.com");
        driver.findElement(By.name("size")).click(); // default selected is small
        driver.findElement(By.cssSelector("input[value='bacon']")).click(); // select topping
        driver.findElement(By.tagName("form")).submit();

        // Wait to let the response load
        Thread.sleep(5000);
